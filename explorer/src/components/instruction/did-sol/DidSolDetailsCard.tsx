...
const parseDidSolInstructionCode = (
  ix: TransactionInstruction
): DidSolInstructionType => ix.data.slice(0, 1).readUInt8(0);
...
function parseDidSolInstruction(
  code: DidSolInstructionType,
  ix: TransactionInstruction
): { [key: string]: any } {
  switch (code) {
    case DidSolInstructionType.Initialize:
  ...
  if (Array.isArray(value)) {
    ...
    return (
      <ul style={{ listStyle: "none" }}>
        <li>{value.id}</li>
        <li>
          <Address pubkey={value.pubkey.toPublicKey()} alignRight link />
        </li>
