...
const parseGatewayInstructionCode = (
  ix: TransactionInstruction
): GatewayInstructionType => ix.data.slice(0, 1).readUInt8(0);

const codeToTitle = (code: GatewayInstructionType) => {
  switch (code) {
    case GatewayInstructionType.AddGatekeeper:
      return "Add Gatekeeper";
    ...
    case GatewayInstructionType.CloseGatekeeper:
      return "Close Gatekeeper";
  }
};
...
function parseGatewayInstruction(
  code: GatewayInstructionType,
  ix: TransactionInstruction
) {
  const { issueVanilla, updateExpiry, setState } =
    GatewayInstruction.decode<GatewayInstruction>(ix.data);
  switch (code) {
    case GatewayInstructionType.Issue:
      return {
    ...
    case GatewayInstructionType.UpdateExpiry:
      return {
        ...
        expireTime: updateExpiry?.expireTime
          ? new Date(updateExpiry?.expireTime * 1000).toISOString()
          : "-",
      };
    case GatewayInstructionType.SetState:
      return {
